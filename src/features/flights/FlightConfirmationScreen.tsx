  ...
  return (
    <div className={`min-h-screen pb-24 pt-8 px-6 max-w-4xl mx-auto transition-colors duration-300 ${appearance === 'Dark Mode' ? 'bg-slate-950 text-white' : 'bg-white text-slate-900'}`}>
      ...
      <div className="space-y-10">
        <h2 className={`text-2xl font-bold ${appearance === 'Dark Mode' ? 'text-white' : 'text-slate-900'}`}>{t('yourFlight')}</h2>

        <div className="space-y-8">
          <div className="flex justify-between items-start gap-4">
            <div className="flex-1 min-w-0">
              <p className="text-sm font-bold text-slate-400 mb-1">{t('date')} : </p>
            ...
            <button onClick={() => onEdit('date')} className="text-red-400 font-bold text-sm shrink-0">{t('edit')}</button>
          </div>

          <div className="flex justify-between items-start gap-4">
            <div className="flex-1 min-w-0">
              <p className="text-sm font-bold text-slate-400 mb-1">{t('from')} : </p>
              <p className={`text-lg font-bold break-words ${appearance === 'Dark Mode' ? 'text-white' : 'text-slate-900'}`}>{flight?.from || 'New York'}</p>
            </div>
            <button onClick={() => onEdit('from')} className="text-red-400 font-bold text-sm shrink-0">{t('edit')}</button>
          </div>

          <div className="flex justify-between items-start gap-4">
            <div className="flex-1 min-w-0">
              <p className="text-sm font-bold text-slate-400 mb-1">{t('to')} : </p>
              <p className={`text-lg font-bold break-words ${appearance === 'Dark Mode' ? 'text-white' : 'text-slate-900'}`}>{flight?.to || 'Tokyo'}</p>
            </div>
            <button onClick={() => onEdit('to')} className="text-red-400 font-bold text-sm shrink-0">{t('edit')}</button>
          </div>

          <div className="flex justify-between items-start gap-4">
            <div className="flex-1 min-w-0">
              <p className="text-sm font-bold text-slate-400 mb-1">{t('flight')} : </p>
              <p className={`text-lg font-bold break-words ${appearance === 'Dark Mode' ? 'text-white' : 'text-slate-900'}`}>{flight?.airline || 'Japan Airlines'}</p>
            </div>
            <button onClick={() => onEdit('flight')} className="text-red-400 font-bold text-sm shrink-0">{t('edit')}</button>
          </div>

          <div>
            <div className="flex justify-between items-center mb-4">
              <p className="text-sm font-bold text-slate-400">{t('guests')} :</p>
            ...
            <p className={`text-lg font-bold mb-2 ${appearance === 'Dark Mode' ? 'text-white' : 'text-slate-900'}`}>{guests} {t('guests')}</p>
            <div className="flex items-center gap-2 text-slate-400 text-xs">
              <Info size={14} className="text-blue-600" />
              <p>{t('guestPriceNote')}</p>
            </div>
          </div>
        </div>
