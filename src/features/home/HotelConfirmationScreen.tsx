  ...
  return (
    <div className={`fixed inset-0 z-[100] flex flex-col transition-colors duration-300 ${appearance === 'Dark Mode' ? 'bg-slate-950 text-white' : 'bg-white text-slate-900'}`}>
      ...
      <div className="flex-1 px-8 pt-6 overflow-y-auto no-scrollbar pb-32">
        <h2 className={`text-2xl font-bold mb-10 ${appearance === 'Dark Mode' ? 'text-white' : 'text-slate-900'}`}>{t('yourHotel')}</h2>

        <div className="space-y-10">
          {/* Date */}
          <div className="flex justify-between items-start">
            <div>
              <p className="text-slate-400 text-sm mb-2">{t('date')} :</p>
              <p className={`text-xl font-bold ${appearance === 'Dark Mode' ? 'text-white' : 'text-slate-900'}`}>10 May</p>
            </div>
            <button onClick={() => onEdit('date')} className="text-red-500 font-bold text-sm active:scale-95 transition-all">{t('edit')}</button>
          </div>

          {/* Hotel */}
          <div className="flex justify-between items-start">
            <div>
              <p className="text-slate-400 text-sm mb-2">{t('hotel')} :</p>
              <p className={`text-xl font-bold ${appearance === 'Dark Mode' ? 'text-white' : 'text-slate-900'}`}>{hotel.name}</p>
            </div>
            <button onClick={() => onEdit('hotel')} className="text-red-500 font-bold text-sm active:scale-95 transition-all">{t('edit')}</button>
          </div>

          {/* Arrangements */}
          <div className="flex justify-between items-start">
            <div>
              <p className="text-slate-400 text-sm mb-2">{t('arrangements')} :</p>
              <p className={`text-xl font-bold ${appearance === 'Dark Mode' ? 'text-white' : 'text-slate-900'}`}>{t('bedroom')} 1</p>
            </div>
            <button onClick={() => onEdit('arrangements')} className="text-red-500 font-bold text-sm active:scale-95 transition-all">{t('edit')}</button>
          </div>
          ...
          <div>
            <div className="flex justify-between items-center mb-4">
              <div>
                <p className="text-slate-400 text-sm mb-2">{t('guests')} :</p>
                <p className={`text-xl font-bold ${appearance === 'Dark Mode' ? 'text-white' : 'text-slate-900'}`}>{guests} {t('guests')}</p>
              </div>
