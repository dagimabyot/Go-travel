...
import { Plane, Bell, QrCode, ChevronLeft, Star, MapPin, Calendar, Hotel, Car } from 'lucide-react';
...
export const HistoryScreen = ({ bookings, user, onCancel, onSelect, onBack, onProfile, language, appearance, savedBookings, toggleSavedBooking }: HistoryScreenProps) => {
        ...
        <AnimatePresence mode="wait">
          {filteredBookings.length === 0 ? (
            ...
            >
              ...
              <h3 className={`text-lg font-bold mb-2 ${appearance === 'Dark Mode' ? 'text-white' : 'text-slate-900'}`}>No {timeFilter} {activeTab}s</h3>
              <p className="text-sm text-slate-500 max-w-[200px]">You don't have any {timeFilter} bookings in this category yet.</p>
            </motion.div>
          ) : (
            <motion.div 
              key={activeTab}
                  ...
                  {activeTab === 'flight' ? (
                    <div className={`rounded-[32px] p-6 shadow-md border transition-colors duration-300 relative ${appearance === 'Dark Mode' ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-100'}`}>
                      ...
                      <div className="flex justify-center mb-4">
                        <span className="text-[10px] font-bold text-primary uppercase tracking-widest">100% on time</span>
                      </div>
                      <div className="flex justify-between items-center mb-6">
                        <div className="text-center">
                          ...
                          <p className="text-[10px] text-slate-500 font-bold">Larkrow</p>
                        </div>
                    ...
                    <div className="relative aspect-[4/3] rounded-[40px] overflow-hidden shadow-lg">
                      ...
                      <div className="absolute bottom-8 left-8 right-8">
                        ...
                        {activeTab === 'hotel' && (
                          <p className="text-white/80 text-sm font-medium">${booking.price}<span className="text-xs opacity-60">/per day</span></p>
                        )}
                      </div>
                    </div>
                  )}
